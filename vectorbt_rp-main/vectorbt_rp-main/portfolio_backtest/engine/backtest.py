...
class BacktestEngine:
    ...
    def run(
        ...
        Args:
            ...
            start_date: 回测开始生效日期（不含），之前的预热期数据将被过滤。
                        策略仍需要 start_date 之前 lookback 天数的数据来计算首个权重。
                        例如：start_date='2024-12-01'，策略 lookback=120，
                             则输入策略的数据从 2024-08-03 开始，但回测结果从 2024-12-01 开始。
        ...
        # 如果指定了 start_date，截取数据
        start_ts = pd.Timestamp(start_date) if start_date is not None else None
        if start_date is not None:
            lookback = getattr(strategy, 'lookback', 0)
            # 策略需要从 start_date - lookback 开始的数据来计算首个权重
            warmup_start = start_ts - pd.Timedelta(days=lookback)
            # 截取数据：保留 warmup_start 之后的数据（用于策略计算）
            price_df = price_df[price_df.index >= warmup_start]
        ...
        # 如果指定了 start_date，过滤回测起始点
        if start_date is not None:
            start_ts = pd.Timestamp(start_ts if start_date is not None else common_index[0])
            # 只保留 start_date 之后的数据
            mask = price_df_aligned.index >= start_ts
            price_df_aligned = price_df_aligned[mask]
            daily_weights_aligned = daily_weights_aligned[mask]
            # 同样过滤 weights_df（调仓日权重）
            weights_df = weights_df[weights_df.index >= start_ts]
