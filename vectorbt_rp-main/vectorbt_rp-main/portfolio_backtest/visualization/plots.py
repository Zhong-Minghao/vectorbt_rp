...
class BacktestVisualizer:
    ...
    def __init__(self, result: BacktestResult, start_date: Optional[pd.Timestamp] = None):
        """
        初始化可视化器

        Args:
            result: BacktestResult 对象
            start_date: 可视化起始日期（不含），之前的预热期数据将被隐藏
        """
        self.result = result
    ...
    def plot_summary(self, show: bool = True, return_fig: bool = False) -> Optional[go.Figure]:
        ...
        # 如果设置了 start_date，直接裁剪数据（xaxis_range 在 vectorbt 中不生效）
        if self.start_date is not None:
            start_ts = pd.Timestamp(self.start_date)
            for trace in fig.data:
                mask = pd.to_datetime(trace.x) >= start_ts
                trace.x = [trace.x[i] for i, m in enumerate(mask) if m]
                trace.y = [trace.y[i] for i, m in enumerate(mask) if m]
